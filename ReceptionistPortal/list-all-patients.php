...
while ($row = $result->fetch_assoc()) {
    ...
    if ($row['Patients_ID'] == $_GET['Patients_ID']) {
        ...
        echo '<td><input type="text" class="form-control" name="PhoneNumber" value="' . $row['PhoneNumber'] . '" required></td>';
    ...
    echo '<button type="button" class="btn btn-primary btn-sm" style="margin-left:10px;" data-bs-toggle="modal" data-bs-target="#ModalView" id="' . $row['Patients_ID'] . '" onClick="showDetailsPatient(this);" title="View"><i class="fas fa-eye"></i></button>';
    echo '<a class="btn btn-danger btn-sm" style="margin-left:10px;" href="delete-patient.php?Patients_ID=' . $row['Patients_ID'] . '" title="Delete" role="button"><i class="fas fa-trash-alt"></i></a>';
    echo '</td>';
    echo '</tr>';
