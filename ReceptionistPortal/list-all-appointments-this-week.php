...
// 1. Capture the contents of the db (sql) and only show this weeks records
$sql = "SELECT * FROM appointments WHERE AppointmentDate BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 7 DAY) ORDER BY AppointmentDate ASC";
...
while ($row = $result->fetch_assoc()) {
    ...
    if ($row['Appointments_ID'] == $_GET['Appointments_ID']) {
        ...
        echo '<form class="form-inline m-2" action="update-appointment-dash.php" method="POST">';
        ...
        echo '<td>';
        echo '<a class="btn btn-info btn-sm" title="Edit" href="dashboard.php?Appointments_ID=' . $row['Appointments_ID'] . '" role="button">Edit</a>';
    ...
    echo '<a class="btn btn-danger btn-sm" href="delete-appointment-dash.php?Appointments_ID=' . $row['Appointments_ID'] . '" title="Delete" role="button">Delete</a>';
